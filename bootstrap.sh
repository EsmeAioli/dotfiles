...
if command -v distrobox &>/dev/null && [[ -d "$CONTAINERS_DIR" ]]; then
  ...
  for ini in "$CONTAINERS_DIR"/*.ini; do
    name=$(basename "$ini" .ini)
    info "Creating container: $name"
    distrobox assemble create --file "$ini" || warn "Failed to create container $name"
    setup="${ini%.ini}-setup.sh"
    if [[ -f "$setup" ]]; then
      info "Running setup script for $name..."
      bash "$setup"
    fi
  done
else
